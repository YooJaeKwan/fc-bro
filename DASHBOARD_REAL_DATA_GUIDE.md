...
```json
{
  ...
  "data": {
    ...
    "recentStats": {
      "attendanceRate": 73,     // 모든 일정의 평균 출석률
      "totalSchedules": 8       // 총 일정 수
    },
    "topAttendancePlayers": [   // 실제 출석률 기반 상위 5명
      {
        "name": "홍길동",
        "position": "ST",
        "attendanceRate": 95,
...
3. 참석 투표 → 출석률 및 참석 현황 실시간 반영 ✨
4. 일정 완료 → 개인별 출석률 업데이트 → 출석왕 순위 변동 ✨
```

### 새로고침 기능:
```
👥 총 팀원 카드 → 자동 업데이트
📈 평균 출석률 → 투표 반영 즉시 업데이트  
...
- ✅ **실제 출석 데이터**: 참석 투표 기반 출석률  
- ✅ **실제 순위 데이터**: 개인별 출석률 기반 순위
